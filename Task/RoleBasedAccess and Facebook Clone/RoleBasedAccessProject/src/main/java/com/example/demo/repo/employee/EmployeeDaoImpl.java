...
public class EmployeeDaoImpl implements EmployeeDao {
    ...
    public Map<String, Object> save(Map<String, Object> jsonData) {
        ...
        String sql =
                "INSERT INTO employee (emp_name, emp_email, product_id, emp_role, site, is_active, status, date_of_joining) " +
        ...
        String loginSql =
                "INSERT INTO loginTable (username, password, role, product_name) " +
    ...
    public void update(Map<String, Object> jsonData) {
        ...
        String updateEmpSql =
                "UPDATE employee SET emp_id='" + emp_id + "', emp_name='" + emp_name + "', emp_email='" + emp_email + "', " +
                "product_id='" + product_id + "', emp_role='" + emp_role + "', site='" + site + "', is_active='" + is_active + "', " +
                "status='" + status + "', date_of_joining='" + date_of_joining + "' WHERE id='" + id + "'";
        ...
        String updateLoginSql =
                "UPDATE loginTable SET role='" + emp_role + "', product_name='" + product_name + "', username='" + emp_name + "' WHERE username='" + oldName + "'";
    ...
    public List<Map<String, Object>> findAll() {
        ...
        String fetchSql =
                "SELECT e.id, e.emp_id, e.emp_name, e.emp_email, e.product_id, p.product_name, e.emp_role, e.site, " +
                "e.date_of_joining, e.is_active, e.status, e.profile_pic, " +
                "CASE " +
                ...
                "FROM employee e LEFT JOIN product p ON e.product_id = p.id ORDER BY e.id";
    ...
    public void saveFromLogin(String username) {
        jdbcTemplate.update(
                "INSERT INTO employee (emp_name, emp_role, is_active, status) VALUES ('" +
                        username + "', 'EMPLOYEE', false, 'IN_PROGRESS')"
        );
    }
    ...
    public Map<String, Object> findByEmail(String empEmail) {
        return jdbcTemplate.queryForMap(
                "SELECT * FROM employee WHERE emp_email='" + empEmail + "'"
        );
    }
    ...
    public List<Map<String, Object>> getRevenueByProduct() {
        String sql =
                "SELECT p.product_name as product, SUM(r.revenue_amount) as total_revenue " +
                "FROM revenue r JOIN product p ON r.product_id = p.id " +
    ...
    public List<Map<String, Object>> getProductPerformance(String productName) {
        String sql =
                "SELECT TO_CHAR(r.month_year, 'Mon YYYY') as month, r.month_year as date, " +
                "SUM(r.revenue_amount) as revenue, SUM(r.expense_amount) as expenses, SUM(r.profit) as profit " +
                "FROM revenue r JOIN product p ON r.product_id = p.id " +
                "WHERE p.product_name='" + productName + "' " +
                "GROUP BY r.month_year ORDER BY r.month_year";
    ...
    public List<Map<String, Object>> getYearComparison(String productName) {
        String sql =
                "SELECT EXTRACT(YEAR FROM r.month_year) as year, COUNT(*) as sales " +
                "FROM revenue r JOIN product p ON r.product_id = p.id " +
                "WHERE p.product_name='" + productName + "' " +
                "GROUP BY EXTRACT(YEAR FROM r.month_year) ORDER BY year";
    ...
    public List<Map<String, Object>> getTopClients(String productName) {
        String sql =
                "SELECT r.client_name as client, SUM(r.revenue_amount) as total_revenue, COUNT(*) as transaction_count " +
                "FROM revenue r JOIN product p ON r.product_id = p.id " +
                "WHERE p.product_name='" + productName + "' AND r.client_name IS NOT NULL " +
                "GROUP BY r.client_name ORDER BY total_revenue DESC LIMIT 5";
    ...
    public Map<String, Object> getMonthlyGrowth(String productName) {
        String sql =
                ...
                "  SELECT r.month_year, SUM(r.revenue_amount) as revenue " +
                "  FROM revenue r JOIN product p ON r.product_id = p.id " +
                "  WHERE p.product_name='" + productName + "' " +
                "  GROUP BY r.month_year ORDER BY r.month_year DESC LIMIT 2 " +
                ") " +
                "SELECT " +
    ...
    public List<Map<String, Object>> getMonthlySalesDistribution(String productName) {
        ...
        String sql =
                "SELECT TO_CHAR(r.month_year, 'Mon') as month, EXTRACT(MONTH FROM r.month_year) as month_num, " +
                "COUNT(*) as sales_count, SUM(r.revenue_amount) as total_revenue " +
                "FROM revenue r JOIN product p ON r.product_id = p.id " +
                "WHERE p.product_name='" + productName + "' " +
    ...
    public List<Map<String, Object>> getCompanyMonthlySales() {
        ...
        String sql =
                "SELECT TO_CHAR(r.month_year, 'Mon') as month, EXTRACT(MONTH FROM r.month_year) as month_num, " +
                "COUNT(*) as sales_count, SUM(r.revenue_amount) as total_revenue " +
                "FROM revenue r WHERE EXTRACT(YEAR FROM r.month_year)=EXTRACT(YEAR FROM CURRENT_DATE) " +
                "GROUP BY TO_CHAR(r.month_year,'Mon'), EXTRACT(MONTH FROM r.month_year) " +
                "ORDER BY month_num";
    ...
    @Override
    public int updateProfilePic(Long empId, String imageUrl) {
        String sql =
                "UPDATE employee SET profile_pic='" + imageUrl + "' WHERE id='" + empId + "'";
    ...
    public String getProfilePic(Long id) {
        return jdbcTemplate.queryForObject(
                "SELECT COALESCE(profile_pic, '') FROM employee WHERE id='" + id + "'",
                String.class
        );
    }
}
...
//    @Override
//    public int updateProfilePic(Long empId, String imageUrl) {
//        String sql = "UPDATE employee SET profile_pic = ? WHERE id = ?";
//        return jdbcTemplate.update(sql, imageUrl, empId);
//    }
//
//
//    @Override
//    public String getProfilePic(Long id) {
//        return jdbcTemplate.queryForObject(
//                "SELECT COALESCE(profile_pic, '') FROM employee WHERE id = ?",
//                String.class,
//                id
//        );
//    }
