...
public class LoginController {
	...
	// UPDATED: Allow login regardless of is_active status
	// The blocking happens at UI level (Masters/Dashboard components)
	@PostMapping("/login")
	public ResponseEntity<?> login(@RequestBody Map<String, Object> jsonData) {
		...
		try {
			...
			if (pw != null && pw.equals(password)) {
				// Remove the is_active check here
				// Just hide password and return user data
	...
	public ResponseEntity<?> checkUsername(@PathVariable String username) {
		try {
			...
			System.out.println("User exists: " + exists); // Debug log
	...
	public ResponseEntity<?> getCurrentUser(@RequestParam String username) {
		try {
			...
			if (user != null) {
				...
				// ✅ Get is_active status from employee table
